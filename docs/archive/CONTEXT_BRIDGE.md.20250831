...
| **🔴 requireAuth 패턴** | requireAuth() 함수 사용 | 1. 즉시 중단 (프로젝트에 없는 함수)<br>2. getUser 패턴으로 변경<br>3. 28개 기존 보호 파일과 동일 패턴 적용 | API Route Agent |
| **🔴 22개 테이블 백업** | 한번에 전체 처리 | 1. 단계적 접근 필수<br>2. CORE(4개) → EXTENDED(7개) → FULL(22개)<br>3. TypeScript 컴파일러 부하 최소화 | Database Agent |
...
## 🚨 **Phase 3 Critical Risk 패턴** (2025-08-30 추가)

**상황**: Phase 3 Quality Improvement 지시서 작성 시 발견된 Critical Risk들이 실제 작업에서도 반복될 가능성이 높음

### 🔴 **Critical Risk #1: API 인증 패턴 충돌**
**❌ 위험한 패턴**: `requireAuth()` 함수 사용
```typescript
// ❌ 절대 금지 - 프로젝트에 존재하지 않는 함수
import { requireAuth } from '@/lib/api-auth';
const user = await requireAuth(request);
```

**✅ 올바른 패턴**: `supabase.auth.getUser()` (프로젝트 표준)
```typescript
// ✅ 프로젝트 표준 - 28개 기존 보호 파일과 동일
...
**🎯 해결책**: `/src/app/api/CLAUDE.md`의 기존 패턴 확인 후 적용

### 🔴 **Critical Risk #2: monitoring.ts any 타입 정의**
**❌ 위험한 패턴**: 6개 any 타입 임시 정의 (18-24줄)
```typescript
// ❌ 절대 금지 - 임시방편 any 타입들
type Alert = any;
type AlertRule = any;
...
**✅ 올바른 패턴**: 구체적 YouTube 모니터링 타입 완전 정의
```typescript
// ✅ 완전한 타입 정의 
interface Alert {
  id: string;
...
**🎯 해결책**: Context7으로 YouTube Data API v3 공식 문서 참조하여 정확한 타입 정의

### 🟡 **Critical Risk #3: 동적 테이블 22개 한번에 처리**
**❌ 위험한 패턴**: 모든 테이블을 Union Type으로 한번에 처리
```typescript
// ❌ TypeScript 컴파일러 부하 과다
export type TableName = keyof Database['public']['Tables']; // 22개 테이블 전체
export const BACKUP_TABLES: TableName[] = [/* 22개 테이블 나열 */];
```

**✅ 올바른 패턴**: 단계적 백업 접근법
```typescript
// ✅ 단계적 접근으로 컴파일러 부하 최소화
export const CORE_BACKUP_TABLES = ['users', 'youtube_favorites', 'collections', 'collection_items'] as const;
export const EXTENDED_BACKUP_TABLES = [...CORE_BACKUP_TABLES, 'yl_channels', 'yl_videos', 'source_folders'] as const;
export const BACKUP_TABLES = CORE_BACKUP_TABLES; // 1단계: 핵심 테이블만 우선
```

**🎯 해결책**: 필요시 점진적으로 EXTENDED → FULL로 확장

### ⚠️ **Pre-commit Hook 도입 주의사항**
**❌ 위험한 접근**: 즉시 전체 강제 적용
- 기존 워크플로우 급격한 변경
- 개발자 적응 시간 부족
- 예상치 못한 차단으로 인한 생산성 저하

**✅ 안전한 접근**: 점진적 단계별 도입
1. **1단계**: VS Code 설정 (위험도 낮음, 부작용 없음)
2. **2단계**: 1주일 경고 모드 운영
3. **3단계**: 팀 합의 후 차단 모드 전환

**🎯 목표**: 안전한 품질 향상 vs 급격한 워크플로우 변경 위험 회피
