...
## 🔥 snake_case/camelCase 일관성 체크리스트 (2025-01-31 최우선)

### 커밋 전 필수 검증
- [ ] **일관성 검증 실행**: `node scripts/verify-case-consistency.js`
  - display_name 오염: 0건 확인
  - snake_case JSX 속성: 0건 확인
  - API 클라이언트 변환 로직 구현 확인
  - Pre-commit Hook 설정 확인

### API 작업 시 확인
- [ ] **API 응답**: snake_case → camelCase 자동 변환 확인
- [ ] **API 요청**: camelCase → snake_case 자동 변환 확인
- [ ] **React 예약어**: displayName, className 등 보호 확인
- [ ] **외부 API**: `skipCaseConversion: true` 옵션 사용
...
- [ ] **인덱스 생성** (외래키, 자주 조회하는 컬럼)
- [ ] **트리거 설정** (updated_at 자동 업데이트)
- [ ] **타입 재생성**: `npm run types:generate`
- [ ] **타입 매핑 함수 업데이트** (snake_case → camelCase)
...
### TypeScript 빌드 체크리스트 (2025-08-21 Wave 2 업데이트)

- [ ] `any` 타입 사용 여부 검사 (**ESLint 에러 0개 유지**)
- [ ] TypeScript 타입 체크: `npm run types:check` (**63개 에러 - Wave 2 진행 중**)
  - ✅ Wave 1: 변수명 오타 42개 해결 (collectionId → collection_id 등)
  - ✅ Wave 2: 타입 정의 확장 완료 (Course, Lesson, CourseProgress, VideoStats, RevenueProof)
  - ⏳ Wave 3: 나머지 타입 오류 해결 필요 (63개)
- [ ] API 함수 반환 타입 명시 확인
- [ ] ZodError.issues 사용 확인 (`.errors` 아님)
- [ ] unknown 타입 적절한 처리 (타입 가드 사용)
- [ ] npm run build 테스트 (현재 타입 오류로 실패)
