        ...
        <div className="space-y-4">
          {events.map((event) => (
            <div key={event.id} className="border rounded-lg p-4">
              ...
              <p className="text-sm text-muted-foreground mb-3">강사: {event.instructor}</p>
              ...
              <Button className="w-full mt-4" disabled={event.registeredCount >= event.maxCapacity}>
                {event.registeredCount >= event.maxCapacity ? '정원 마감' : '수강 신청하기'}
              </Button>
            </div>
          ))}
        </div>
