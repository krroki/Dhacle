            ...
            <div className="bg-card rounded-lg border p-6">
              ...
              <div className="grid grid-cols-7 gap-1">
                {calendarDays.map((dayInfo, index) => {
                  const isHovered = hoveredDate === dayInfo.day && dayInfo.isCurrentMonth && dayInfo.hasEvents;
                  const isToday = dayInfo.isToday;
                  
                  return (
                    <button
                      key={index}
                      onClick={() => dayInfo.isCurrentMonth && dayInfo.hasEvents && setSelectedDate(dayInfo.day)}
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
              {monthSchedules.length === 0 ? (
                ...
                monthSchedules.map((schedule) => {
                  ...
                  const isSelected = selectedDate === parseInt(day);
                  const { instructor, title } = formatCourseName(schedule.courseName);
                  
                  return (
                    <button
                      key={schedule.id}
                      onMouseEnter={() => setHoveredDate(parseInt(day))}
                      ...
                      className={`
                        ...
                        ${hoveredDate === parseInt(day) ? 'transform scale-[1.02]' : ''}
                      `}
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <ChevronRight className={`
                          ...
                          ${hoveredDate === parseInt(day) ? 'translate-x-1' : ''}
                        `} />
                      </div>
                    </button>
                  );
