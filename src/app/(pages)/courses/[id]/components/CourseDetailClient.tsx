...
export function CourseDetailClient({ courseData }: CourseDetailClientProps) {
  ...
  const getLessonIcon = (lesson: typeof lessons[0], index: number) => {
    if (lesson.is_free) return <PlayCircle className="w-4 h-4 text-green-600" />;
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm ml-1">
                    {course.average_rating.toFixed(1)} ({course.review_count}개)
                  </span>
                </div>
              ...
              <section id="intro">
                ...
                <ContentBlocks blocks={
                  typeof course.content_blocks === 'string' 
                    ? JSON.parse(course.content_blocks) 
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg">{course.instructor_name}</h3>
                        <p className="text-muted-foreground mt-2">
                          YouTube Shorts 전문 크리에이터로 100만 구독자를 보유하고 있습니다.
                          5년간의 콘텐츠 제작 경험을 바탕으로 실전 노하우를 전달합니다.
                        </p>
                      </div>
                    </div>
                  </CardContent>
