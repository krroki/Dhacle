...
const adjectives = [
  ...
  '상큼한', '달콤한', '깔끔한', '산뜻한', '포근한',
  '든든한', '씩씩한', '당당한', '우아한', '화려한',
  '조용한', '차분한', '성실한', '부지런한', '열정적인',
  '창의적인', '도전적인', '긍정적인', '활기찬', '명랑한',
  '쾌활한', '유쾌한', '상냥한', '친절한', '정직한',
  '겸손한', '지혜로운', '현명한', '영리한', '재치있는',
  '센스있는', '매력적인', '사랑스런', '깜찍한', '발랄한'
];
 ...
 * @returns 포맷된 닉네임
 */
export function formatNickname(nickname: string, maxLength: number = 10): string {
...
export function getNicknameType(profile: {
  ...
  if (profile.naver_cafe_verified && profile.naver_cafe_nickname) {
    return 'naver';
  }
...
export function getDisplayNickname(profile: {
  ...
  if (profile.naver_cafe_verified && profile.naver_cafe_nickname) {
    return profile.naver_cafe_nickname;
  }
  if (profile.random_nickname) {
    return profile.random_nickname;
  }
  if (profile.username) {
