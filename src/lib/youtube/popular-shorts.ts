...
import type { PopularShortsParams, VideoWithStats } from '@/types';
...
export async function getPopularShortsWithoutKeyword(
  ...
  const filtered = videos_with_metrics.filter((video) => {
    ...
    return (stats.view_count ?? 0) >= minViews && (stats.viewsPerHour || 0) >= minVPH;
  });
...
async function store_videos_in_database(videos: VideoWithStats[]): Promise<void> {
  ...
  try {
    ...
    const video_data = videos.map((v) => ({
      ...
      thumbnail_url: v.thumbnail_url || null, // Use the existing thumbnail_url field
      tags: v.tags,
